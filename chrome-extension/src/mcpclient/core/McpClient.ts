...
export class McpClient extends EventEmitter<AllEvents> {
  ...
  private normalizeTools(toolPrimitives: Primitive[]): NormalizedTool[] {
    return toolPrimitives.map(p => {
      ...
      // Get input schema and ensure it has a valid type
      let inputSchema = tool.inputSchema || tool.input_schema;
      
      // Ensure schema is a valid object
      if (typeof inputSchema !== 'object' || inputSchema === null || Array.isArray(inputSchema)) {
        inputSchema = { type: 'object' };
        logger.debug(`[McpClient] Invalid schema for tool: ${tool.name}, using default`);
      }
      
      // Ensure schema has a type property (AJV requirement)
      if (!inputSchema.type) {
        logger.debug(`[McpClient] Adding missing 'type' to schema for tool: ${tool.name}`);
